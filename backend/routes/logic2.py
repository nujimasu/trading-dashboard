...
@router.get("/api/logic2-picks")
def get_logic2_picks():
    conn = get_connection()
    cur  = conn.cursor()
    cur.execute("""
    ...
    for r in rows:
        ...
        h4_triggers_all = json.loads(r.get("h4_triggers_all") or "[]")
        ...
        result.append({
            ...
            "technical_summary": {
                ...
                "risk_factors":      [
                    ...
                    f"トリガーボーナス: +{trigger_bonus:.2f}" if trigger_bonus > 0 else None,
                ],
            },
