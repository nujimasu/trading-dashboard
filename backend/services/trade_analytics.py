...
def get_insights() -> dict:
    ...
    # 自動カードに source を付与
    for c in cards:
        c.setdefault("source", "auto")
        c.setdefault("pinned", False)

    # カスタム (Claude/手動) カードを取得して結合
    custom = _get_custom_cards()
    # 並び順: pinned 優先 → custom (新しい順) → auto
    pinned    = [c for c in custom if c.get("pinned")]
    unpinned  = [c for c in custom if not c.get("pinned")]
    merged    = pinned + unpinned + cards
    ...
    if not merged:
        merged.append({
            ...
            "metrics": [], "source": "auto", "pinned": False,
        })
    return {"cards": merged}


def _get_custom_cards() -> list[dict]:
    """custom_insights テーブルから未 dismiss のカードを取得。"""
    with db_cursor() as cur:
        cur.execute("""
            SELECT id, title, body, severity, icon, metrics, tags, source, pinned, created_at
    ...
    for r in rows:
        ...
        if isinstance(tags, str):
            try:    tags = json.loads(tags)
                ...
                tags = [s.strip() for s in tags.strip("{}").split(",") if s.strip()]
        out.append({
            "id":       f"custom_{r['id']}",
            "db_id":    r["id"],
            "title":    r["title"],
            ...
            "metrics":  metrics or [],
            "tags":     tags or [],
            "source":   r.get("source") or "claude",
            "pinned":   bool(r.get("pinned")),
        })
    return out


def add_custom_insight(payload: dict) -> dict:
    """カスタムインサイトを追加。"""
    title    = (payload.get("title") or "").strip()
    body     = (payload.get("body") or "").strip()
    if not title or not body:
        raise ValueError("title と body は必須です")
    ...
    with db_cursor() as cur:
        cur.execute("""
            INSERT INTO custom_insights
              (title, body, severity, icon, metrics, tags, source, pinned)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        """, (title, body, severity, icon,
              json.dumps(metrics, ensure_ascii=False),
              json.dumps(tags if isinstance(tags, list) else [tags], ensure_ascii=False),
              source, pinned))
    return {"ok": True}


def dismiss_custom_insight(insight_id: int) -> dict:
    with db_cursor() as cur:
        cur.execute("""
            UPDATE custom_insights
...
def toggle_pin_custom_insight(insight_id: int) -> dict:
    with db_cursor() as cur:
        cur.execute("SELECT pinned FROM custom_insights WHERE id = ?", (insight_id,))
        row = cur.fetchone()
        if not row:
            raise ValueError("insight not found")
        current = bool(row["pinned"])
        cur.execute("UPDATE custom_insights SET pinned = ? WHERE id = ?",
                    (not current, insight_id))
    return {"ok": True, "pinned": not current}
